...
function createParticles(x, y, count, color, size, life, speedSpread, gravity) {
    for (let i = 0; i < count; i++) {
        ...
        particles.push(new Particle(x, y, color, pSize, speedX, speedY, pLife, pGravity));
    }
}
...
if (laser.charging) {
    ...
    const chargeSize = (laser.chargeCounter / laser.chargeTime) * (player.size + chargePulse);
    const alpha = laser.chargeCounter / laser.chargeTime;

    // Emit charging particles
    createParticles(player.x + player.size, player.y + player.size / 2, 2, `rgba(255, 100, 100, ${alpha})`, Math.random() * 5 + 1, 30, 10, 0);

    ctx.save();
    ctx.globalCompositeOperation = "lighter";
    ctx.fillStyle = `rgba(255, 0, 0, ${0.4 * alpha})`;
    ctx.beginPath();
    ctx.arc(player.x + player.size, player.y + player.size / 2, chargeSize, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
...
if (laser.firing) {
    ...
    createParticles(laser.beam.x, laser.beam.y + laser.beam.height / 2, 5, "rgba(255, 255, 255, 0.7)", 1, 30, 8, 0);
    createParticles(laser.beam.x, laser.beam.y + laser.beam.height / 2, 3, "rgba(255, 100, 100, 0.8)", 1.5, 20, 10, 0);
    ...
    ctx.fillRect(laser.beam.x, laser.beam.y, laser.beam.width, laser.beam.height);
    
    // Inner white core
    ctx.shadowBlur = 0;
    ctx.fillStyle = "rgba(255, 255, 255, 0.7)";
    ctx.fillRect(laser.beam.x, laser.beam.y + 5, laser.beam.width, laser.beam.height - 10);
...
// --- End laser charging and firing logic ---
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        if (laser.firing && laser.beam) {
            if (
                ...
                // Beam hit particles
                createParticles(obs.x + obs.width / 2, obs.y + obs.height / 2, 10, "rgba(255, 255, 255, 0.9)", 2, 15, 5, 0);
    ...
    if (hitboxesVisible) {
        ...
        if (laser.firing && laser.beam) {
            ctx.strokeRect(laser.beam.x, laser.beam.y, laser.beam.width, laser.beam.height);
        }
    ...
    if (laser.cooldownCounter > 0) {
        fillPercentage = (laser.cooldownTime - laser.cooldownCounter) / laser.cooldownTime;
...
document.addEventListener("keydown", (e) => {
    ...
    if (e.code === "KeyX" && gameRunning && laser.cooldownCounter === 0 && !laser.charging && !laser.firing) {
        laser.charging = true;
    }
    if (e.code === "KeyH") {
...
function activateBomb(collectedBomb) {
    ...
    explode(collectedBomb.x + collectedBomb.size / 2, collectedBomb.y + collectedBomb.size / 2);
